...
use crate::tensor::{TensorMut, Tensors};
use crate::{Error, Map};
...
impl Connection {
    ...
    /// Register a tensor with the larod daemon for server-side fd tracking.
    ///
    /// This is useful when callers provide an external fd through
    /// [`TensorMut::set_fd`]. The caller is still responsible for keeping the fd
    /// valid according to the larod API contract.
    pub fn track_tensor(&self, tensor: &mut TensorMut<'_>) -> Result<(), Error> {
        let (success, maybe_error) =
            unsafe { try_func!(larod_sys::larodTrackTensor, self.raw, tensor.as_mut_ptr()) };
        if success {
            Ok(())
        } else {
            Err(maybe_error.unwrap_or(Error::MissingError))
