...
impl<'a> TensorRef<'a> {
    ...
    pub fn fd_props(&self) -> Result<u32, Error> {
        let mut props: u32 = 0;
        let (success, maybe_error) = unsafe {
            try_func!(
                larod_sys::larodGetTensorFdProps,
                self.raw as *const _,
                &mut props,
            )
        };
...
impl<'a> TensorMut<'a> {
    ...
    pub fn set_fd_props(&mut self, props: u32) -> Result<(), Error> {
        let (success, maybe_error) =
            unsafe { try_func!(larod_sys::larodSetTensorFdProps, self.raw, props) };
        if success {
            Ok(())
        } else {
            Err(maybe_error.unwrap_or(Error::MissingError))
    ...
    pub(crate) fn as_mut_ptr(&mut self) -> *mut larod_sys::larodTensor {
        self.raw
    }
}
...
mod tests {
    ...
    #[test]
    fn unassigned_fd_is_none() {
        assert!(matches!(super::decode_fd(-1, None), Ok(None)));
    }
